            ...
            <div class="layui-card">
                <div class="layui-card-body ">
                    <form class="layui-form layui-col-space5" method="post"
                          action="${pageContext.request.contextPath}/enterprise/showAllStudent">
                        <div class="layui-input-inline layui-show-xs-block">
                            <input type="text" name="stuId" autocomplete="off" placeholder="请输入学号"
                                   class="layui-input"/>
                        </div>
                        <div class="layui-input-inline layui-show-xs-block">
                            <input type="text" name="stuEntrance" autocomplete="off" placeholder="请输入入学年份"
                                   class="layui-input"/>
                        </div>
                        <div class="layui-input-inline layui-show-xs-block">
                            <select name="stateId">
                ...
                <c:if test="${sessionScope.manager!=null&&sessionScope.enterprise==null}">
                    <div class="layui-card-header">
                        ...
                        <button class="layui-btn"
                                onclick="xadmin.open('添加学生','${pageContext.request.contextPath}/student/forwardToAddStu',500,550)">
                    ...
                    <table class="layui-table layui-form">
                        ...
                        <c:forEach items="${pager.datas}" var="student">
                            <tr>
                                ...
                                <td>
                                    <c:out value="${student.stuEntrance}"/>
                                </td>
                                <td>
                                    <c:if test="${student.stateId == 0}">
                                        未就业
                                    </c:if>
                                    <c:if test="${student.stateId == 1}">
                                        求职中
                                    </c:if>
                                    <c:if test="${student.stateId == 2}">
                                        已就业
                                    </c:if>
                                    <c:if test="${student.stateId == 3}">
                                        离职
                                    </c:if>
                                </td>
                                ...
                                <td class="td-manage">
                                    <button class="layui-btn layui-btn layui-btn-xs"
                                            onclick="xadmin.open('查看详细信息','${pageContext.request.contextPath}/manager/selectStuById?stuId=${student.stuId}&flag=0',700,500)">
                                        <i class="layui-icon">&#xe642;</i>查看详细信息
                                    </button>
                                    <c:if test="${sessionScope.manager!=null&&sessionScope.enterprise==null}">
                                        <button class="layui-btn layui-btn layui-btn-xs"
                                                onclick="xadmin.open('修改','${pageContext.request.contextPath}/manager/selectStuById?stuId=${student.stuId}&flag=1',700,500)">
...
<script>
    ...
    function uploadFile() {
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/import/importStuExcel",
            type: "POST",
            data: myForm,
            contentType: false,
            processData: false,
